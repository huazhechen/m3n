...
分页视图以 Open Fanqie A4 的 `1000 × 1415` 为基准。应用会将用户选择的乐谱宽度直接写入 Open Fanqie 页面配置；为使简谱与 Verovio 五线谱在相同页宽下保持接近的视觉比例，简谱另以 `0.8` 的视觉尺度调整字形、字体、边距和行距，而不改变页面宽度或以虚拟宽度重排。连续模式会按系统数扩展 SVG viewBox 高度。因用户选择保留这些响应式行为，非基准宽度不是像素级 Open Fanqie A4 基准。
