...
分页视图以 Open Fanqie A4 的 `1000 × 1415` 为基准。应用的乐谱宽度会直接写入 Open Fanqie 页面配置，重新计算可打印宽度、自动断行和页高；连续模式会按系统数扩展 SVG viewBox 高度。因用户选择保留这些响应式行为，非基准宽度不是像素级 Open Fanqie A4 基准。
