...
import { renderOpenFanqieScore } from './open-fanqie-score.js'

describe('renderOpenFanqieScore', () => {
  it('uses the Open Fanqie page, glyph, layout, and font contract from a ScoreDocument', () => {
    const document = parseM3NDocument('{title=测试曲} {key=D} {3/4} {90qpm}\nN: ||: 1. 2 3 | 4~ 5 6 :|||{x3}\nL: 春天来 | 了')
    const [svg] = renderOpenFanqieScore(document, { paged: true, width: 1000 })
  ...
  it('wraps the Lao Nan Hai corpus score into multiple Open Fanqie systems', () => {
  ...
  it('reflows systems when the configured page width changes', () => {
    ...
    const systemsAt = (width: number) => {
      const [svg] = renderOpenFanqieScore(document, { paged: false, width })
  ...
  it('projects ScoreMeasure endings as Open Fanqie volta brackets', () => {
    const document = parseM3NDocument('{2/4}\nN: ||: 1 2 |\n---V1\nN: 3 4 :||\n---V2\nN: 5 6 |||')
    const [svg] = renderOpenFanqieScore(document, { paged: false, width: 1000 })
    ...
    expect(bracket).toContain('<line x1="192.5"')
    expect(svg).toContain('<use x="190.5" y="142" xlink:href="#xiaojiexian"')
  })
  ...
  it('renders every complete beat after a long note as an Open Fanqie sustain glyph', () => {
  ...
  it('keeps all simultaneous chord pitches as vertically stacked Fanqie glyphs', () => {
    const document = parseM3NDocument('{2/4}\nN: [1d 3d 5d:h] 0 |||')
    const [svg] = renderOpenFanqieScore(document, { paged: false, width: 1000 })
    ...
    const ys = [...svg.matchAll(/<use x="([\d.]+)" y="([\d.]+)" xlink:href="#shuzi_b_[135]"/g)]
      .filter((match) => match[1] === '83')
      .map((match) => Number(match[2]))
    expect(new Set(ys).size).toBe(3)
    expect(ys[0]! - ys[1]!).toBe(30)
    expect(ys[1]! - ys[2]!).toBe(30)
    const lowOctaveDotYs = [...svg.matchAll(/<use x="[\d.]+" y="([\d.]+)" xlink:href="#yingao_di"/g)].map((match) => Number(match[1]))
    expect(lowOctaveDotYs).toContain(ys[0]! + 1)
    expect(lowOctaveDotYs).toContain(ys[1]! + 1)
    expect(lowOctaveDotYs).toContain(ys[2]! + 1)
  })

  it('expands ScoreDocument tuplets into compact Open Fanqie note groups', () => {
    const document = parseM3NDocument('{4/4}\nN: [123:2] 4 5 |||')
    const [svg] = renderOpenFanqieScore(document, { paged: false, width: 1000 })
    const notes = [...svg.matchAll(/<use x="([\d.]+)" y="([\d.]+)" xlink:href="#shuzi_b_([123])"[^>]*data-m3n-id="m3n-e-1"/g)]

    expect(notes.map((match) => match[1])).toEqual(['83', '108', '133'])
  ...
  it('uses the compact tempo glyph and the staff-score credit priority', () => {
    ...
    expect(svg).toContain('xlink:href="#jiepaifu" transform="translate(80,216) scale(0.78)')
  ...
  it('paginates by the rendered lyric rows so Guang Yin De Gu Shi lyrics remain visible', () => {
    ...
    const pages = renderOpenFanqieScore(parseM3NDocument(source), { paged: true, width: 1000 })
  ...
  it('justifies an automatically wrapped Hong He Gu first system at 930 width', () => {
    ...
    const [svg] = renderOpenFanqieScore(parseM3NDocument(source), { paged: true, width: 930 })

    expect(svg).toContain('<use x="853" y="275" xlink:href="#xiaojiexian"')
  })

  it('combines contiguous M3N legato intervals into one Open Fanqie line', () => {
  ...
  it('starts a volta at the system edge when its first measure starts a new system', () => {
    ...
    expect(svg).toMatch(/<use x="83" y="[^\"]+" xlink:href="#xiaojiexian"[^>]*code="\|"/)
    expect(svg).toMatch(/<line x1="85" y1="[^\"]+" x2="85" y2="[^\"]+"/)
  })
  ...
  it('uses the first lyric row for generic L: after numbered lyric passes', () => {
    ...
    const [svg] = renderOpenFanqieScore(parseM3NDocument(source), { paged: false, width: 800 })
