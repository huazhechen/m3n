...
import {
  ...
  normalizeAdjacentBarlines,
  parseM3NDocument,
  validateM3NDiagnostics,
  formatScoreDiagnostic,
  type ScoreDiagnostic,
} from '@m3n/notation'

export type Happi123ConversionResult = {
  source: string
  output: string
  diagnostics: string[]
...
function validateM3N(source: string, options: { skipBeatValidation?: boolean } = {}) {
  return validateM3NDiagnostics(source, options).map(formatScoreDiagnostic)
}
...
/**
 * The converter historically emitted the v0.2 "free body" form. Keep that
 * form readable for callers, but expose proper v0.3 rows so the current
 * analyser can retain phrase-scoped lyrics and source mapping.
 */
function modernizeOutput(source: string, hasParts: boolean) {
  const lines = source.split('\n')
  const lyricStart = lines.findIndex((line) => /^\{lyrics(?:=\d+)?\}$/.test(line.trim()))
  const bodyEnd = lyricStart >= 0 ? lyricStart : lines.length
  const metadata = /^(?:\{(?:title|subtitle|singer|composer|lyricist|key|parts)=|\{\d+\/\d+\}|\{\d+qpm\})/
  const musicStart = lines.findIndex((line, index) => index < bodyEnd && line.trim() !== '' && !metadata.test(line.trim()))
  if (musicStart < 0) return source
  const header = lines.slice(0, musicStart).filter((line) => !/^\{parts=/.test(line.trim()))
  const music = lines.slice(musicStart, bodyEnd).join(' ').replace(/\s+/g, ' ').trim()
  const renderRows = (value: string, section?: string) => {
    const rows: string[] = []
    const voltaPattern = /\{volta=([^}]+)\}([\s\S]*?)\{\/\}/g
    let cursor = 0
    for (const match of value.matchAll(voltaPattern)) {
      const start = match.index ?? 0
      const before = value.slice(cursor, start).replace(/\s+/g, ' ').trim()
      if (before && /^(?::?\|\|+)$/.test(before) && rows.length > 0) rows[rows.length - 1] += ` ${before}`
      else if (before) rows.push(`N: ${before}`)
      const passes = (match[1] ?? '').replace(/[~-]/g, ',')
      const ending = (match[2] ?? '').replace(/\s+/g, ' ').trim()
      rows.push(`---V${passes}\nN: ${ending}`)
      cursor = start + match[0].length
    }
    const trailing = value.slice(cursor).replace(/\s+/g, ' ').trim()
    if (trailing && /^(?::?\|\|+)$/.test(trailing) && rows.length > 0) rows[rows.length - 1] += ` ${trailing}`
    else if (trailing) rows.push(`N: ${trailing}`)
    if (rows.length === 0) rows.push(`N: ${value}`)
    return `${section ? `===${section}\n` : ''}${rows.join('\n')}`
  }
  const partRows = hasParts
    ? [...music.matchAll(/\{part=([^}]+)\}/g)].map((match, index, matches) => {
      const start = (match.index ?? 0) + match[0].length
      const end = matches[index + 1]?.index ?? music.length
      const content = music.slice(start, end)
        .replace(/^\s*\{\/\}\s*/, '')
        .replace(/\s*\{\/\}\s*$/, '')
        .trim()
      return renderRows(content, match[1]?.trim())
    }).join('\n')
    : ''
  const musicRows = partRows || renderRows(music)
  const lyrics: string[] = []
  if (lyricStart >= 0) {
    const tail = lines.slice(lyricStart).join('\n')
    for (const match of tail.matchAll(/\{lyrics(?:=(\d+))?\}\s*([\s\S]*?)\s*\{\/\}/g)) {
      const label = match[1] ? `L${match[1]}` : 'L'
      lyrics.push(`${label}: ${(match[2] ?? '').trim()}`)
    }
  }
  return [...header, musicRows, ...lyrics].join('\n')
}
...
export function happi123ToM3N(source: string): Happi123ConversionResult {
  ...
  const output = modernizeOutput(
    repairLegacyStructure(normalizeMeasureMeters(repairedOutput), diagnostics)
      .replace(/\(\{(\d+\/64)\}\s*/g, '{$1} ('),
    partCount > 0,
  )
  ...
  return {
    ...
    diagnosticDetails: uniqueDiagnostics.map((message) => ({
      code: 'HAPPI123_CONVERSION',
      severity: 'error',
      message,
    })),
  }
