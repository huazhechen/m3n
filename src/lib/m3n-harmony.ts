...
export function m3nChord(value: string, key: string): M3NChord | null {
  const match = /^(VII|III|II|IV|VI|V|I|vii|iii|ii|iv|vi|v|i)(m|dim|aug|sus2|sus4|[2-9]|1[0-3])?$/.exec(value)
  ...
  const quality = suffix === 'dim' || suffix === 'aug' || suffix === 'sus2' || suffix === 'sus4'
  ...
  const intervals = /^(?:m)?(7|9|11|12|13)$/.test(quality) ? [0, third, fifth, 10] : [0, third, fifth]
