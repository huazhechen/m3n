...
import { createScoreDiagnostic, formatScoreDiagnostic } from './diagnostics'
...
describe('diagnostic localization', () => {
  ...
  it('keeps compatibility messages for rules not yet migrated', () => {
    const diagnostic = createScoreDiagnostic({ code: 'M3N_VALIDATION', message: '旧规则', legacyMessage: '第 2 行：旧规则' })
